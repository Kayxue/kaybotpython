...
class Bot(commands.Bot):
    def __init__(self):
        super(Bot, self).__init__(
            command_prefix=["s!", "sesame "],
            help_command=None,
            description="由芝麻湯圓所製作的機器人",
        )
    ...
    async def on_ready(self):
        await self.change_presence(
            ...
            activity=discord.Game(name="s!help / sesame help"),
        )
        print("Bot is online")
...
class maintask(Cog_Extension):
    ...
    async def load(self, ctx, extension):
        ...
        embed1 = discord.Embed(
            title="指定類別指令載入成功！",
            description=f"成功載入{extension}類別指令！",
            color=0xB6B8BA,
        )
    ...
    async def unload(self, ctx, extension):
        ...
        embed1 = discord.Embed(
            title="指定類別指令解除載入成功！",
            description=f"成功解除載入{extension}類別指令！",
            color=0xB6B8BA,
        )
        await ctx.send(embed=embed1)
    ...
    async def reload(self, ctx, extension):
        ...
        embed1 = discord.Embed(
            title="已重新載入指定類別指令！",
            description=f"成功重新載入{extension}類別指令！",
            color=0xB6B8BA,
        )
        await ctx.send(embed=embed1)
    ...
    async def reload_error(self, ctx, error):
        if isinstance(error, discord.ext.commands.CheckFailure):
            ...
            embed1 = discord.Embed(
                title="重新載入失敗！",
                description="對不起，不明原因，無法重新載入該類別之指令",
                color=0xB6B8BA,
            )
            embed1.add_field(name="錯誤碼", value=error, inline=False)
            await ctx.channel.send(embed=embed1)
    ...
    async def help(self, ctx):
        embed = [
            discord.Embed(
                title="幫助－資訊類", description="資訊類指令表", color=0xB6B8BA
            )
            ...
            discord.Embed(
                title="幫助－休閒娛樂類", description="休閒娛樂類指令表", color=0xB6B8BA
            )
            .add_field(name="say", value="讓bot說出指定訊息", inline=False)
            .add_field(
                name="saydel",
                value="讓bot說出指定訊息（輸入之指令會被刪除）",
                inline=False,
            )
            .add_field(name="loly", value="隨機顯示一張蘿莉圖"),
            discord.Embed(
                title="幫助－伺服器資訊類",
                description="伺服器資訊類指令表，\n所有人能執行這些指令喔！",
                color=0xB6B8BA,
            )
            .add_field(name="serverinfo", value="查看此伺服器的資訊", inline=False)
            .add_field(
                name="serverboost", value="查看此伺服器之加速狀態", inline=False
            ),
            discord.Embed(
                title="幫助－伺服器管理類",
                description="伺服器管理類指令表，\n要執行此類指令必須要有該指令相對應之權限",
                color=0xB6B8BA,
            )
            .add_field(
                name="clear",
            ...
            discord.Embed(
                title="幫助－機器人管理類",
                description="機器人管理類指令表，\n僅湯圓本人能執行這些指令喔！\n這些為管理機器人載入之功能與指令的指令",
                color=0xB6B8BA,
            )
            .add_field(name="load", value="啟用指定類別之功能或指令", inline=False)
            .add_field(
                name="reload", value="重新載入指定類別之功能或指令", inline=False
            )
