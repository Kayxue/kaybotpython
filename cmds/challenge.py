...
class Challenge(Cog_Extension):
    ...
    async def makechallenge(self, ctx, game: str, level: str, info: str, endtime: str):
        async with aiofiles.open("count.json", mode="r", encoding="utf8") as jfile:
            file = await jfile.read()
            jdata = json.load(file)
        async with aiofiles.open("challenge.json", mode="r", encoding="utf8") as jfile:
            file = await jfile.read()
        ...
        async with aiofiles.open("count.json", mode="w", encoding="utf8") as finish:
            parsed_str = json.dumps(jdata, sort_keys=True, indent=4, ensure_ascii=False)
            await finish.write(parsed_str)
        outmsg = await channel.send(
            content=f"{(discord.utils.get(ctx.guild.roles, name='Event')).mention}挑戰已開始！",
            embed=embed1,
        )
        challenges[str(jdata["challengecount"])]["messageid"] = outmsg.id
        async with aiofiles.open("challenge.json", mode="w", encoding="utf8") as finish:
            parsed_str = json.dumps(
                challenges, sort_keys=True, indent=4, ensure_ascii=False
            )
            await finish.write(parsed_str)
    ...
    async def makechallenge_error(self, ctx, error):
        if isinstance(error, discord.ext.commands.CheckFailure):
            embed1 = discord.Embed(
                title="權限不足！", description="您沒有權限執行此指令！", color=0xB6B6BA
            )
            ...
            embed1.add_field(
                name="用法",
                value="``s!makechallenge [遊戲名稱] [挑戰關卡] [挑戰內容] [截止時間]``",
            )
            embed1.add_field(
                name="範例",
                value="``s!makechallenge 滾動的天空 雷鬼 0石3冠 2100年7月8日``",
            )
            await ctx.channel.send(embed=embed1)
        else:
            embed1 = discord.Embed(
                title="輸出失敗！", description="對不起，無法執行！", color=0xB6B8BA
            )
    ...
    async def challengeinfo(self, ctx, chalid: str):
        try:
            async with aiofiles.open(
                "challenge.json", mode="r", encoding="utf8"
            ) as jfile:
                file = await jfile.read()
                challenges = json.load(file)
    ...
    async def endchallenge(self, ctx, chalid: str):
        try:
            ...
            if challenges[chalid]["status"] == "Starting":
                channel = discord.utils.get(ctx.guild.channels, name="挑戰challenge")
                host = ctx.guild.get_member(int(challenges[chalid]["host"]))
                messagetoedit = await channel.fetch_message(
                    int(challenges[chalid]["messageid"])
                )
                challenges[chalid]["status"] = "Ended"
                async with aiofiles.open(
                    "challenge.json", mode="w", encoding="utf8"
                ) as finish:
                    parsed_str = json.dumps(
                        challenges, sort_keys=True, indent=4, ensure_ascii=False
                    )
