        ...
        if howmanyinateam >= 2:
            ...
            if not dotimes < 1:
                ...
                embed1 = discord.Embed(
                    title="組隊結果", description="以下為組隊結果", color=0xB6B8BA
                )
                ...
                await ctx.channel.send(embed=embed1)
            elif dotimes < 1 and teamok is True:
                await ctx.channel.send(
                    f"上線人數過少，無法分出{team}組並且一組{howmanyinateam}人"
                )
            elif dotimes < 1 and teamok is False:
                await ctx.channel.send(f"上線人數過少，無法分出一組{howmanyinateam}人")

        else:
            await ctx.channel.send("請輸入一隊有幾人（此數字必須大於等於2）")
