...
class Info(Cog_Extension):
    ...
    async def botinfo(self, ctx):
        embed1 = discord.Embed(
            title="關於此機器人", description="關於此機器人的資訊", color=0xB6B8BA
        )
    ...
    async def userinfo(self, ctx):
        embed1 = discord.Embed(
            title="使用者資訊", description="關於此使用者的資訊", color=0xB6B8BA
        )
        ...
        embed1.add_field(
            name="加入此伺服器日期：",
            value=ctx.author.joined_at.strftime("%Y/%m/%d %p %I:%M:%S %Z"),
            inline=False,
        )
        embed1.add_field(
            name="帳戶創建日期：",
            value=ctx.author.created_at.strftime("%Y/%m/%d %p %I:%M:%S %Z"),
            inline=False,
        )
    ...
    async def weather(self, ctx, location: str):
        ...
        embed1 = discord.Embed(
            title="天氣資訊", description="該地天氣資訊如下：", color=0xB6B8BA
        )
