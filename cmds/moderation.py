...
class Moderation(Cog_Extension):
    ...
    async def clear(self, ctx, num: int):
        ...
        embed1 = discord.Embed(
            title="清理訊息成功！", description=f"成功清除{num}則訊息！", color=0xB6B8BA
        )
    ...
    async def clear_error(self, ctx, error):
        if isinstance(error, discord.ext.commands.CheckFailure):
            ...
            embed1 = discord.Embed(
                title="清除失敗！", description="對不起，無法清除", color=0xB6B8BA
            )
    ...
    async def kick(self, ctx, user: discord.Member, *, reason: str = "未填寫"):
        ...
        embed1 = discord.Embed(
            title="踢出成員", description="指定成員已成功被踢出！", color=0xB6B8BA
        )
        ...
        embed1.set_footer(
            text=ctx.message.author, icon_url=ctx.message.author.avatar_url
        )
        await user.kick(reason=reason)
        embed2 = discord.Embed(
            title="成功踢出成員！",
            description=f"已成功踢出成員：{user.mention}！",
            color=0xB6B8BA,
        )
    ...
    async def kick_error(self, ctx, error):
        if isinstance(error, discord.ext.commands.CheckFailure):
            ...
            embed1 = discord.Embed(
                title="踢出失敗！", description="對不起，找不到此成員"
            )
            await ctx.channel.send(embed=embed1)
        elif isinstance(error, discord.ext.commands.MissingRequiredArgument):
            embed1 = discord.Embed(
            ...
            embed1.add_field(
                name="用法", value="``s!kick [提及欲踢出之成員] [原因（選用）]``"
            )
            embed1.add_field(name="範例", value="``s!kick @Dancing Sky 測試``")
            await ctx.channel.send(embed=embed1)
        else:
            embed1 = discord.Embed(
                title="踢出失敗！", description="對不起，無法踢出成員", color=0xB6B8BA
            )
            embed1.add_field(name="錯誤訊息：", value=error)
            await ctx.channel.send(embed=embed1)
    ...
    async def ban(self, ctx, user: discord.Member, *, reason: str = "未填寫"):
        channel = discord.utils.get(user.guild.channels, name="sesame-log")
        embed1 = discord.Embed(
            title="封鎖成員", description="指定成員已成功被封鎖！", color=0xB6B8BA
        )
        ...
        embed2 = discord.Embed(
            title="成功封鎖成員！",
            description=f"已成功封鎖成員：{user.mention}！",
            color=0xB6B8BA,
        )
        await channel.send(embed=embed1)
        await ctx.channel.send(embed=embed2)
    ...
    async def ban_error(self, ctx, error):
        if isinstance(error, discord.ext.commands.CheckFailure):
            ...
            embed1 = discord.Embed(
                title="封鎖失敗！", description="對不起，找不到此成員"
            )
            await ctx.channel.send(embed=embed1)
        elif isinstance(error, discord.ext.commands.MissingRequiredArgument):
            embed1 = discord.Embed(
            ...
            embed1.add_field(
                name="用法", value="``s!ban [提及欲封鎖之成員] [原因（選用）]``"
            )
            embed1.add_field(name="範例", value="``s!ban @Dancing Sky 測試``")
            await ctx.channel.send(embed=embed1)
        else:
            embed1 = discord.Embed(
                title="封鎖失敗！", description="對不起，無法封鎖成員", color=0xB6B8BA
            )
            embed1.add_field(name="錯誤訊息：", value=error)
            await ctx.channel.send(embed=embed1)
