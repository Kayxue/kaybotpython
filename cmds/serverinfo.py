...
class ServerInfo(Cog_Extension):
    ...
    async def serverinfo(self, ctx):
        ...
        if safety["verification level"] == discord.VerificationLevel.none:
            ...
            safety["verification level"] == discord.VerificationLevel.high
            or safety["verification level"] == discord.VerificationLevel.table_flip
        ):
            safety["verification level"] = "高驗證層級"
        elif (
            safety["verification level"] == discord.VerificationLevel.extreme
            or safety["verification level"]
        ...
        embed1 = discord.Embed(
            title=f"關於伺服器「{ctx.guild.name}」",
            description=f"關於伺服器{ctx.guild.name}的資訊",
            color=0xB6B8BA,
        )
        embed1.add_field(name="伺服器ID", value=f"{ctx.guild.id}", inline=False)
        embed1.add_field(
            name=f"成員［{ctx.guild.member_count}］：",
    ...
    async def serverboost(self, ctx):
        embed1 = discord.Embed(
            title=f"關於伺服器{ctx.guild.name}之加成資料",
            description="關於此伺服器現在之加成資料",
            color=0xB6B8BA,
        )
        premiums = {
            "level": ctx.guild.premium_tier,
            "boost user count": ctx.guild.premium_subscription_count,
        }
